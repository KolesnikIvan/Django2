...
# при изменении заказа будет меняться остаток на складе
def product_quantity_update_on_order_item_save(sender, update_fields, instance, **kwargs):
    # sender - это класс-источник сигнала, instance - сохраняемый объект
    # возможно, следовало обернуть в транзакцию
    # renew store product residuals
